...
import { Plus, Search, MapPin, Edit2, Database } from "lucide-react";
    ...
    <div className="p-6 space-y-6 bg-secondary-50/20 min-h-screen">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <h1 className="text-3xl font-bold text-secondary-900 leading-tight mb-2 tracking-tight">Location Master</h1>
          <p className="text-secondary-500 font-medium">Coordinate storage zones and facility infrastructure</p>
        </motion.div>
        <Button
          onClick={handleAdd}
      ...
      <Card className="shadow-sm border-secondary-200/60 bg-white">
        <div className="p-5 flex items-center">
          <div className="relative flex-1 w-full">
            ...
            <Input
              placeholder="Filter by zone name, company alignment, or code..."
      ...
      <Card className="shadow-xl shadow-secondary-200/20 border-secondary-200/60 overflow-hidden bg-white">
        <div className="p-6 border-b border-secondary-100 flex items-center justify-between bg-gradient-to-r from-white to-secondary-50/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-secondary-900 tracking-tight">Infrastructure Ledger</h3>
              <p className="text-[11px] font-bold text-secondary-400 uppercase tracking-widest">{filteredLocations.length} Mapped Areas</p>
            </div>
          </div>
        </div>
          ...
          <Table>
            <TableHeader className="bg-secondary-50/50">
              <TableRow className="border-secondary-100">
                ...
                <TableHead className="font-bold text-secondary-600 uppercase tracking-widest text-[10px]">Facility Zone</TableHead>
                <TableHead className="font-bold text-secondary-600 uppercase tracking-widest text-[10px]">Enterprise Alignment</TableHead>
                    ...
                    >
                      ...
                      <TableCell className="py-5">
                        <div className="flex flex-col">
                          <span className="font-bold text-secondary-900 text-sm group-hover:text-primary-700 transition-colors uppercase tracking-tight">{loc.name}</span>
                          <span className="text-[10px] font-bold text-secondary-400 uppercase tracking-tight">Prime Infrastructure</span>
                        </div>
                      </TableCell>
                      <TableCell className="py-5 font-bold text-secondary-600 text-sm italic">{loc.company?.name || 'GENERIC ENTITY'}</TableCell>
                      <TableCell className="py-5 text-center">
                        <span className={`inline-flex items-center px-3 py-1 rounded-xl text-[10px] font-bold uppercase tracking-wider border shadow-sm ${loc.isActive
                          ? 'bg-emerald-50 text-emerald-700 border-emerald-100'
                          : 'bg-rose-50 text-rose-700 border-rose-100'
                          }`}>
                          <div className={`w-1.5 h-1.5 rounded-full mr-2 ${loc.isActive ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
                    ...
                    <TableCell colSpan={5} className="py-24 text-center bg-secondary-50/20">
                      <div className="flex flex-col items-center justify-center space-y-4">
                        ...
                        <div className="space-y-1">
                          <p className="text-secondary-900 font-bold text-lg">No Locations Detected</p>
                          <p className="text-secondary-400 text-sm font-medium">We couldn't find any location records matching your search criteria.</p>
                        </div>
                        <Button variant="outline" onClick={() => setSearch("")} className="mt-4 font-bold border-secondary-300 rounded-xl px-6">Refresh Infrastructure Ledger</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                )}
