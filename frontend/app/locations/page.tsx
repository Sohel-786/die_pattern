...
export default function LocationsPage() {
    ...
    return (
      <div className="flex h-[80vh] items-center justify-center font-sans px-4">
        <div className="text-center p-8 bg-white rounded-3xl shadow-xl border border-secondary-100 max-w-sm">
          ...
          <p className="text-secondary-500 font-medium">You don't have the required clearance to view locations.</p>
        </div>
      </div>
    );
  }
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => api.put(`/locations/${selectedItem?.id || data.id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["locations"] });
