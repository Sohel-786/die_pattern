...
interface JwItemSelectionDialogProps {
    open: boolean;
    onClose: () => void;
...
export function JwItemSelectionDialog({
    open,
    onClose,
    selectedItemIds,
    onAddItems,
}: JwItemSelectionDialogProps) {
    const [search, setSearch] = useState("");
    /** Items the user has selected in this dialog to be added to the Job Work (pending commit). */
    ...
    const { data: allItems = [], isLoading } = useQuery<Item[]>({
        queryKey: ["items", "active", "in-stock-selection"],
        queryFn: async () => {
    ...
    const filtered = useMemo(() => {
        ...
        return allItems.filter((i) => {
            ...
            const isNotAlreadySelected = !selectedSet.has(i.id);
            return matchesSearch && isNotAlreadySelected;
        });
    }, [allItems, search, selectedSet]);

    const addToPending = (item: Item) => {
    ...
    const commitAdd = () => {
        ...
        setPendingItems([]); // Clear pending after adding
    };
    ...
    return (
        <Dialog
            isOpen={open}
            onClose={handleDone}
            title="Select Die / Pattern (In Stock)"
            size="3xl"
            contentScroll={false}
        ...
        >
            <div className="flex flex-col flex-1 min-h-0 px-6 py-4 gap-4 bg-white dark:bg-background">
                <p className="text-[11px] font-bold text-secondary-500 dark:text-secondary-400 uppercase tracking-widest leading-relaxed">
                    Add available items to your Job Work selection. You can add multiple batches without closing.
                </p>
                                ...
                                <TableBody>
                                    {pendingItems.map((item, idx) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-1.5 text-secondary-600 dark:text-gray-300 font-bold text-xs uppercase tracking-tighter">{item.mainPartName}</td>
                                            <td className="py-1.5">
                ...
                <div className="flex-1 min-h-0 border border-secondary-200 dark:border-border rounded-xl overflow-hidden bg-white dark:bg-card shadow-sm flex flex-col">
                    ...
                    {isLoading ? (
                        <div className="flex-1 flex flex-col items-center justify-center py-20 gap-3 bg-white dark:bg-background">
                            ...
                            <p className="font-black uppercase tracking-[0.2em] text-[10px] text-secondary-400 dark:text-secondary-600 animate-pulse">Scanning Inventory...</p>
                        </div>
                    ) : (
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={4} className="py-24 text-center">
                                                <div className="flex flex-col items-center gap-2 opacity-40 dark:opacity-20 text-secondary-600 dark:text-secondary-400">
                                                    ...
                                                    <p className="font-black uppercase tracking-widest text-[10px]">No matching items found in stock</p>
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    ) : (
