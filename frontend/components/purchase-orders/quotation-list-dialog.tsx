...
export interface QuotationListDialogProps {
  open: boolean;
  onClose: () => void;
...
export function QuotationListDialog({
  open,
  onClose,
  urls,
  urlsToDelete,
  ...
  return (
    <>
      <Dialog isOpen={open} onClose={onClose} title="Uploaded quotations" size="md">
        <div className="space-y-3 max-h-[60vh] overflow-auto">
          {total === 0 ? (
            <p className="text-sm text-secondary-500 py-4 text-center">No quotations added yet.</p>
          ) : (
            <>
              {displayUrls.map((url) => (
                <div
      ...
      {viewFile && (
        <BlobQuotationViewer file={viewFile} onClose={() => setViewFile(null)} />
      )}
    </>
  );
}
